...
open class AnimalServiceClient {
    ...
    open func getAlligators(_ success: @escaping ([Animal]) -> Void, failure: @escaping (NSError?) -> Void) {
        Alamofire.request("\(baseUrl)/alligators")
            .responseJSON {
                result in
                if result.result.isSuccess {
                    if let jsonArray = result.result.value as? [[String: AnyObject]] {
    ...
    open func getSecureAlligators(authToken: String, success: @escaping ([Animal]) -> Void, failure: @escaping (NSError?) -> Void) {
    ...
    open func getAlligator(_ id: Int, success: @escaping (Animal) -> Void, failure: @escaping (NSError?) -> Void) {
        Alamofire.request("\(baseUrl)/alligators/\(id)")
            .responseJSON {
                result in
                if result.result.isSuccess {
                    if let jsonResult = result.result.value as? [String: AnyObject] {
